...
public class Review {
	public static void main(String[] args) {
		...
		// 문자열변수 = sc.nextLine();							// 문장을 입력 (버그가 있어서 next, nextInt 와 혼용 금지)
		...
		// 자료형 변환
		// int <-> double : 기본 자료형일 경우에는 괄호로 충분
		int num = 4;
		double dNum = 3.14;
		...
		// String <-> int : 개발된 자료형(클래스), 괄호로는 사용할 수 없음
		// toString, Integer.parseInt
		String str = "001230";
		num = Integer.parseInt(str);			// 문자열을 정수로 바꾼거
		str = Integer.toString(num);			// 정수를 문자열로 바꾼거
		...
		for(int i=0; i<5; i++) {
			...
			if(i == 2) {
				break;				// 반복문 끝
				// coutinue;		// 밑에 부분은 실행하지 않고 첫 부분으로 올라감
			}
...
class TheOne{
	private String str;
	public TheOne() {
		// 클래스 이름과 같고 소괄호가 있으면 '생성자', 자료형이 없음(필요가 없음)
		// 생성자 : 객체화하는 순간에 사용될 메서드(함수)
		System.out.println("객체화 성공!");
	}
	public String getStr() {				// 메서드의 자료형 : return의 자료형
		return str;							// return : 함수를 종료시키면서 해당 값을 사용한 곳에 전달하는 역할을 함
	}
	public void setStr(String str) {		// () : 매개변수, 사용하는 곳에서 전달해 줘야하는 값을 지정하는 변수들
		this.str = str;						// this. : '이 클래스의' 뜻으로, 멤버에 있는 변수나 메서드를 사용하기 위해 앞에 붙여줌
	}
...
interface TheTwo{
	public void go();						// 인터페이스의 메서드는 상속받은 곳에서 {}를 구현
